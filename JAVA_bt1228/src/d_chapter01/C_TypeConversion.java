...
//			작은 범위을 타입을 큰 타입으로 넣을 때는 
//			자동으로 형 변환을 컴파일러가 수행
		big_bowl = small_bowl;
		
		byte a = 32; // 1 byte 자료형 
		short b = a; // 1 byte 자료형에 저장된 값을 2 byte 으로 변환 
		int c = b;
		long d = c;
		...
		// : 크기가 작은 자료형으로의 변환 
		int number = 1234; 
		float float_number= number; // int보다  float이 더크다.
...
//		명시적 형 변환: (작은변환자료형)큰데이터값;
		...
		// 실수를 정수로 강제 타입 변환 시 소수형 이하의 데이터가 버려짐
